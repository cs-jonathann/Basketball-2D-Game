...
public abstract class GameBase extends Applet implements Runnable, KeyListener, MouseListener, MouseMotionListener
{
	...
	// Using this to close applet quick
	...
	// implementation is in Game class since Game extends GameBase
	public abstract void inGameLoop();
	
	
	public final void update(Graphics g)
	{
		// wiping screen every frame
		off_screen_g.clearRect(0, 0, 1710, 1080);
	...
	{
		// drawn on this
		off_screen   = createImage(1710, 1080);
		
		// our "pen"
		off_screen_g = off_screen.getGraphics();
			
		
		// applet listening to key presses 
		requestFocus();		
		addKeyListener(this);		
		...
		// passing the game into the thread
		t = new Thread(this);
		
		// launches thread
		t.start();
		
		// without the 2 previous lines, nothing would ever update or draw on screen
	}
